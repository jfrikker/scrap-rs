...
pub fn remove_scopes(module: &mut sir::Module) {
    super::transform_module(module, &|expression| {
        match expression {
...
fn remove_scope(name: &str, value: &sir::Expression, body: &mut sir::Expression) {
    super::transform_expression(body, &|expression| {
        match expression {
            sir::Expression::Reference { name: ref_name } if ref_name == name => *expression = value.clone(),
            _ => {},
        }
    })
}